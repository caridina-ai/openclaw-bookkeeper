...
# bookkeeping/ 是安裝單元（openclaw skills install 的對象），程式在它底下
BOOK = ROOT / "bookkeeping" / "scripts" / "book.py"
SKILL = ROOT / "bookkeeping" / "SKILL.md"
DEFAULT_CATEGORIES = [
...
class BookCLITest(unittest.TestCase):
    ...
    def book_env(self):
        env = os.environ.copy()
        env["BOOKKEEPING_TEST_DB"] = str(self.db)
        env["PYTHONUTF8"] = "1"
        return env
    ...
    def test_fresh_database_has_original_16_categories(self):
        ...
        for index, category in enumerate(DEFAULT_CATEGORIES, start=1):
            ...
            self.assertTrue(output.startswith(f"2026-01-01 #{index} {category} (測試{index}) 1"))
    ...
    def test_used_category_cannot_be_deleted_or_cleared_but_can_be_renamed(self):
        ...
        self.assertEqual(
            ...
            "2026-07-09 12:30 #1 餐飲 (午餐) 120 餘額 -120",
        )
        self.assertEqual(
        ...
        self.assertIn("12:30 #1 伙食 (午餐) 120", detail)
    ...
    def test_time_is_preserved_for_record_edit_delete_and_detail(self):
        self.assertEqual(
            ...
            "2026-07-09 08:05 #1 入金 (現金) 1000 餘額 1000",
        )
        self.assertEqual(
            self.run_book(
            ...
            "2026-07-09 14:55 #2 保健費 (看牙) 350 餘額 650",
        )
        self.assertEqual(
            self.run_book(
            ...
            "2026-07-09 14:55 #2 保健費 (看牙) 350 -> 300 餘額 650 -> 700",
        )
        detail = self.run_book("detail", "--date", "2026-07-09")
        self.assertIn("14:55 #2 保健費 (看牙) 300", detail)
        self.assertNotIn("入金", detail)
        self.assertEqual(
            self.run_book(
                "delete", "--find", "現金", "--date", "2026-07-09 08:05"
            ),
            "2026-07-09 08:05 刪除 #1 入金 (現金) 1000 餘額 700 -> -300",
        )
    ...
    def test_explicit_midnight_remains_visible_when_editing_and_deleting(self):
        ...
        self.assertEqual(
            ...
            "2026-07-09 00:00 #2 外食費 (午夜餐) 100 -> 120 餘額 400 -> 380",
        )
        self.assertEqual(
            self.run_book(
    ...
    def test_edit_and_delete_accept_or_search_terms(self):
        ...
        self.assertEqual(
            ...
            "#2 居住費 (七月台灣自來水帳單) 230 -> 250 餘額 360 -> 340",
        )
        self.assertEqual(
            self.run_book(
                "delete", "--find", "瓦斯", "--find", "自來水", "--amount", "250"
            ),
            "刪除 #2 居住費 (七月台灣自來水帳單) 250 餘額 340 -> 590",
        )
        self.assertEqual(
            self.run_book(
    ...
    def test_multiple_matches_require_rowid_before_edit_or_delete(self):
        ...
        self.assertIn(f"#{ids['七月水費']} | 2026-07-02 09:00 | 居住費 | 七月水費 | 100", matches)
        self.assertIn(f"#{ids['七月水電']} | 2026-07-03 10:00 | 居住費 | 七月水電 | 200", matches)
        self.assertTrue(matches.endswith("請依上述內容選定編號，再執行修改。"))
    ...
    def test_report_category_detail_expand_and_adjust_outputs(self):
        ...
        self.assertIn("2026-07-02 09:10 #2 冰紅茶 25", category_detail)
        expanded = self.run_book(
            "expand", "--category", "保健費", "--month", "2026-07"
        )
        self.assertIn("2026-07-03 14:55 #3 牙醫 300", expanded)
        self.assertIn("保健費 300", expanded)
        self.assertEqual(
    ...
    def test_multiple_months_and_multiple_dates(self):
        ...
        self.assertIn("2026-07-09 09:00 #2 七月早餐 60", category_detail_dates)
        self.assertIn("2026-07-11 18:30 #3 七月藥品 70", category_detail_dates)
        expanded = self.run_book(
        ...
        self.assertIn("09:00 #2 外食費 (七月早餐) 60", dates)
        self.assertIn("18:30 #3 保健費 (七月藥品) 70", dates)
    ...
    def test_category_detail_and_expand_put_subtotal_on_the_category_line(self):
        records = [
            ("2026-07-01 08:00", "外食費", "早餐", "60"),
            ("2026-07-02 12:30", "外食費", "午餐", "2735"),
            ("2026-07-03 14:55", "保健費", "牙醫", "300"),
        ]
        for date, category, note, amount in records:
            self.run_book(
                "out", "--category", category, "--note", note, "--amount", amount,
        ...
        self.assertEqual(
            ...
            "\n".join([
                ...
                "=======",
                "外食費 2795",
                "2026-07-01 08:00 #1 早餐 60",
                "2026-07-02 12:30 #2 午餐 2735",
                "",
                "保健費 300",
                "2026-07-03 14:55 #3 牙醫 300",
                "-------",
                "合計 3095",
            ]),
        )
        self.assertEqual(
            self.run_book("expand", "--category", "外食費", "--month", "2026-07"),
            "\n".join([
                "2026-07",
                "=======",
                "外食費 2795",
        ...
        # 明確指定但整期沒有帳目的科目仍要保留科目行，小計為 0
        self.assertEqual(
            self.run_book("expand", "--category", "旅遊金", "--month", "2026-07"),
            "2026-07\n=======\n旅遊金 0\n-------\n合計 0",
        )
        # 分帳明細的科目小計必須和分帳（report）的同科目金額一致
        report = self.run_book("report", "--month", "2026-07")
        self.assertIn("外食費 2795", report.replace("  ", " "))

    def test_guide_lists_user_phrases_without_leaking_internal_commands(self):
        guide = self.run_book("guide")
        for phrase in (
            "七月分類統計", "七月分類明細", "七月流水帳", "展開七月的外食費",
            "餘額", "起始餘額2000", "列出所有科目", "說明",
        ):
            self.assertIn(phrase, guide)
        # 這份輸出是給使用者看的，不該漏出子指令或旗標
        for internal in ("category-detail", "book.py", "--month", "--category"):
            self.assertNotIn(internal, guide)
        lines = guide.splitlines()
        for index, line in enumerate(lines[:-1]):
            if line and set(line) == {"="}:
                self.assertNotEqual(lines[index + 1], "", guide)
        self.assertNotIn("\n\n\n", guide)
        # 純說明，不需要也不該動到帳本
    ...
    def test_book_is_a_self_contained_script_run_by_uv(self):
        # 其餘測試直接用 sys.executable 叫 book.py（快、且不必每次進 uv）；
        # 這一項才是驗證 SKILL.md 教弱模型的那個呼叫方式真的跑得起來。
        source = BOOK.read_text(encoding="utf-8")
        block = re.search(
            r"^# /// script$\n(?P<body>(?:^#.*$\n)+?)^# ///$", source, re.MULTILINE
        )
        self.assertIsNotNone(block, "book.py 缺少 PEP 723 inline metadata")
        meta = "\n".join(
            line[2:] if line.startswith("# ") else line[1:]
            for line in block.group("body").splitlines()
        )
        self.assertIn("requires-python", meta)
        # 只用標準庫。要加套件就寫進這個區塊，不可另外開 requirements 檔，
        # 因為安裝單元只有 SKILL.md 與 scripts/。
        self.assertRegex(meta, r"dependencies\s*=\s*\[\s*\]")

        skill = SKILL.read_text(encoding="utf-8")
        self.assertIn('uv run "<本 SKILL.md 所在目錄>/scripts/book.py"', skill)

        uv = shutil.which("uv")
        if not uv:
            self.skipTest("找不到 uv，略過實際 uv run 驗證")
        result = subprocess.run(
            [uv, "run", str(BOOK), "guide"],
            cwd=ROOT,
            env=self.book_env(),
            text=True,
            encoding="utf-8",
            capture_output=True,
            check=False,
            timeout=300,  # 首次執行可能要下載直譯器
        )
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertIn("記帳說明：這樣打最快", result.stdout)
