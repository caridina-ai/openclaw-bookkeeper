...
舊檔裡寫的 `.tap` 就是現在的 `.ops`（那時它只裝得下「頻道在哪」）。
歷史文件維持原文不動，那是當時的紀錄。
...
要動這個專案，得先知道的事全在這裡：原始碼在哪、怎麼測、部署到哪、
線上由誰在跑、log 在哪、使用者從哪條頻道進來、上次檢討讀到哪。

> **`.ops` 絕不進版控。** 裡面是私密 id 與本機絕對路徑，換一台機器就是
> 另一份——它描述的是**這台機器上的現場**，不是這份原始碼。
> `.gitignore` 用不帶路徑的 `.ops` 擋，所以放在哪一層都擋得掉。

| 區塊 | 欄位 |
|---|---|
| 原始碼 | `repo` `test` `deploy` |
| 線上實況 | `config` `route` `log` |
| 使用者從哪進來 | `channel` `target` `since` `human` `bot` |

「頻道在哪」只是其中一區。這個專案剛好是聊天驅動的才有那一區；
換成別種專案，前兩區照樣成立。

工具只挑自己認得的鍵（`dump-channel.js` 只讀 `channel`／`target`／`since`／
`bot`），其餘是給人和將來的 chattapper 看的，多放不會壞。
